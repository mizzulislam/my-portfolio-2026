...
import { motion, AnimatePresence, animate, useInView, useMotionValue, useMotionTemplate } from 'motion/react';
...
export function SectionHeader({ title, subTitle, icon: Icon, isDark }: any) {
  return (
    <div className="flex flex-col items-center text-center mb-16 w-full px-4">
      <h2 className={`text-2xl md:text-4xl font-extrabold mb-3 flex flex-wrap items-center justify-center gap-2.5 tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>
      ...
      {subTitle && (
        <p className={`max-w-2xl text-sm md:text-base leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
          {subTitle}
        </p>
      )}
    </div>
...
export function AnimatedCounter({ to, duration = 2 }: { to: number; duration?: number }) {
  const [count, setCount] = useState(0);
  const ref = useRef(null);
  const isInView = useInView(ref, { once: false, margin: "-50px" });
  useEffect(() => {
    if (isInView) {
      const controls = animate(0, to, { duration: duration, ease: "easeOut", onUpdate: (value) => setCount(Math.floor(value)) });
      ...
      <div className="p-8 md:p-10">
        <div className="flex flex-col md:flex-row justify-between gap-5 md:gap-4 mb-8 items-start">
            ...
            <div className={`w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center shrink-0 border transition-all overflow-hidden ${isDark ? 'bg-white border-white/10' : 'bg-white border-slate-200 shadow-sm'}`}>
              {edu.logo ? (
                ...
                <div className={`w-full h-full flex items-center justify-center ${isDark ? 'text-blue-500' : 'text-blue-600'}`}>
                  {edu.id === 'sma' ? <School size={20} className="md:w-6 md:h-6" /> : <GraduationCap size={20} className="md:w-6 md:h-6" />}
                </div>
              )}
            </div>
            <div className="flex flex-col flex-1">
              <h3 className={`text-lg md:text-xl font-bold transition-colors mb-1 ${isDark ? 'text-white group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-blue-400 group-hover:to-cyan-300' : 'text-slate-900 group-hover:text-blue-600'}`}>{edu.institution}</h3>
              <p className={`${isDark ? 'text-slate-300' : 'text-slate-600'} font-medium text-[11px] md:text-sm`}>{edu.degree}</p>
            </div>
          </div>
          ...
          <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-start w-full md:w-auto gap-2 md:gap-1.5 shrink-0">
            <span className={`inline-block px-3 py-1 border rounded-full text-[10px] font-mono font-bold uppercase tracking-wider whitespace-nowrap ${isDark ? 'bg-white/5 border-white/10 text-blue-400' : 'bg-blue-50 border-blue-100 text-blue-600'}`}>{edu.period}</span>
            {edu.gpa && <p className={`text-[10px] font-bold uppercase tracking-tighter md:pr-3 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>GPA: <span className={isDark ? 'text-white' : 'text-slate-900'}>{edu.gpa}</span></p>}
          </div>
        </div>
        <ul className="space-y-3 mb-8">{edu.details.map((detail: string, i: number) => (<li key={i} className="flex items-start gap-3 text-sm leading-relaxed"><CheckCircle2 size={16} className="text-blue-500 shrink-0 mt-1" /><span className={isDark ? 'text-slate-400' : 'text-slate-600'}>{detail}</span></li>))}</ul>
        {edu.courses && (
          <div className={`border-t pt-6 ${isDark ? 'border-white/5' : 'border-slate-100'}`}>
            <button onClick={() => setIsOpen(!isOpen)} className={`flex items-center gap-2 font-bold text-sm hover:text-blue-500 transition-colors group/btn ${isDark ? 'text-white' : 'text-slate-900'}`}><BookOpen size={18} className="text-blue-500" /><span>{labels.coursesBtn}</span><motion.div animate={{ rotate: isOpen ? 180 : 0 }} className="text-slate-500 group-hover/btn:text-blue-500"><ChevronDown size={18} /></motion.div></button>
            <AnimatePresence>{isOpen && (<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden"><div className="grid grid-cols-2 md:grid-cols-3 gap-3 pt-6">{edu.courses.map((course: string, i: number) => (<div key={i} className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-[10px] md:text-xs font-medium transition-all uppercase tracking-wide ${isDark ? 'bg-white/5 border-white/5 text-slate-400 hover:bg-blue-600/10' : 'bg-slate-50 border-slate-100 text-slate-500 hover:bg-blue-50'}`}><div className="w-1.5 h-1.5 bg-blue-500 rounded-full shrink-0" />{course}</div>))}</div></motion.div>)}</AnimatePresence>
          </div>
        )}
      </div>
    </motion.div>
...
export function ExperienceRoadmap({ labels, isDark }: any) {
  ...
  return (
    <div className="relative w-full pb-8 pt-2 px-4 flex flex-col items-center bg-transparent overflow-hidden" ref={containerRef}>
      ...
      <div className="block md:hidden w-full relative">
        ...
        <div className="w-full space-y-16 relative z-10 py-6">
          {milestones.map((m: any, i: number) => {
            ...
            return (
              <div key={i} className={`relative flex items-center w-full px-2 ${isLeft ? 'justify-start' : 'justify-end'}`}>
                {/* Milestone Platform */}
                <motion.div 
                  initial={{ scale: 0 }}
                  whileInView={{ scale: 1 }}
                ...
                >
                  {/* Arrow for mobile */}
                  <div className={`absolute top-1/2 -translate-y-1/2 w-3 h-3 bg-[#161B22] border-white/5 rotate-45 ${isLeft ? '-right-[7px] border-t border-r' : '-left-[7px] border-l border-b'}`} />
                  
                  <span className="text-lg font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-300 mb-1 font-mono text-left">{m.year}</span>
                  <div className="h-[1px] w-6 bg-blue-600/40 mb-2" />
                  <p className="text-[10px] font-medium text-white leading-tight text-left line-clamp-3">{m.detail || m.event}</p>
                </motion.div>
              </div>
            );
          })}
        ...
        <div className="min-w-[1250px] h-[550px] relative flex flex-col justify-center px-16">
          ...
          <div className="flex justify-between items-center w-full relative z-10">
            {milestones.map((m: any, i: number) => {
              ...
              return (
                <div key={i} className="flex flex-col items-center relative group w-48">
                  {/* Milestone Card (Zig-Zag) */}
                  <div className={`absolute left-1/2 -translate-x-1/2 w-64 ${isUp ? 'bottom-[calc(100%+32px)]' : 'top-[calc(100%+32px)]'}`}>
                    <motion.div 
                    ...
                    >
                      {/* Arrow indicator */}
                      <div className={`absolute left-1/2 -translate-x-1/2 w-4 h-4 bg-[#161B22] border-white/5 rotate-45 ${isUp ? 'bottom-[-9px] border-r border-b' : 'top-[-9px] border-l border-t'}`} />
                      
                      <span className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-300 mb-2 font-mono tracking-tighter text-left">{m.year}</span>
                      <div className="h-[2px] w-10 bg-blue-600/40 mb-3" />
                      <p className="text-sm font-medium text-white leading-relaxed text-left whitespace-normal line-clamp-3 overflow-hidden">{m.detail || m.event}</p>
                    </motion.div>
                  </div>
                  ...
                  >
                    <div className="w-14 h-14 rotate-45 rounded-xl border-4 flex items-center justify-center transition-all bg-slate-950 border-blue-600/70 shadow-[0_0_20px_rgba(37,99,235,0.2)] group-hover:border-blue-400 group-hover:scale-110 group-hover:shadow-[0_0_25px_rgba(37,99,235,0.5)]">
                      <NodeIcon size={20} className="text-blue-400 -rotate-45" />
                    </div>
                  </motion.div>
                </div>
              );
...
export function ProjectFolders({ projects, isDark, labels }: any) {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto mt-16 px-4">
            ...
            >
              ...
              >
                <div className="flex items-center gap-2.5">
                  <span className={`font-mono text-[9px] md:text-[10px] font-bold transition-colors duration-300 ${isActive ? 'text-blue-500' : 'text-slate-500'}`}>
                    {String(idx + 1).padStart(2, '0')}
                  </span>
                  <span className={`text-[9px] md:text-xs font-black uppercase tracking-widest whitespace-nowrap transition-colors duration-300 ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-slate-200'}`}>
      ...
      <div className={`relative w-full rounded-tl-3xl rounded-tr-3xl rounded-b-3xl border transition-all duration-700 min-h-[500px] flex items-center ${
        isDark 
          ...
          >
            <div className="space-y-12 text-left w-full">
              ...
              <div className="space-y-8 w-full">
                <div className="flex flex-col items-start gap-4">
                  ...
                  <div className="space-y-6 w-full">
                    <h2 className={`text-2xl md:text-[43px] font-black tracking-tighter leading-tight text-left ${isDark ? 'text-white' : 'text-slate-900'}`}>
                      {activeProject.title}
                    </h2>
                    <p className={`text-[13px] md:text-[17px] font-medium leading-relaxed text-left text-pretty w-full ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
                      {activeProject.desc}
                    </p>
                  </div>
                </div>
                  ...
                  {activeProject.details.map((detail: string, i: number) => (
                    <motion.div 
                      ...
                      className={`flex items-start gap-4 p-5 rounded-2xl border transition-all ${
                        isDark ? 'bg-white/[0.04] border-white/5 hover:bg-white/[0.08]' : 'bg-slate-50/50 border-slate-200 hover:bg-slate-100/50'
                      }`}
                    >
                      <CheckCircle2 size={18} className="text-blue-500 shrink-0 mt-0.5" />
                      <p className="text-[11px] md:text-sm font-bold tracking-tight text-left leading-snug">{detail}</p>
                    </motion.div>
                  ))}
                </div>
              </div>
              ...
              <div className="pt-8 flex justify-end transform -translate-y-3 md:translate-y-0 md:translate-x-4">
                ...
                >
                  {labels?.launchArchive || 'Launch Archive'} <ExternalLink size={15} className="group-hover:translate-x-1 transition-transform" />
                </motion.a>
              </div>
            </div>
          </motion.div>
...
export function SkillLogoBox({ skill, isDark, type, beamX = null, parentRef = null }: any) {
  const boxRef = useRef<HTMLDivElement>(null);
  const [isRevealed, setIsRevealed] = useState(false);

  useEffect(() => {
    if (type === 'tech' && beamX !== null && boxRef.current && parentRef?.current) {
  ...
  const getStyle = (name: any, skillType: string) => {
    ...
    if (skillType === 'tech') {
      if (n.includes('excel') || n.includes('spreadsheet')) return { icon: Table, color: '#10b981' };
      if (n.includes('word')) return { icon: FileText, color: '#3b82f6' };
      if (n.includes('powerpoint') || n.includes('presentation')) return { icon: Presentation, color: '#f97316' };
      if (n.includes('myob') || n.includes('zahir')) return { icon: Calculator, color: '#6366f1' };
      if (n.includes('google')) return { icon: Globe, color: '#ef4444' };
      if (n.includes('spss') || n.includes('e-views') || n.includes('stata')) return { icon: BarChart3, color: '#a855f7' };
    ...
    if (skillType === 'soft') {
      if (n.includes('communicat') || n.includes('komunikasi')) return { icon: MessageSquare, color: '#6366f1' };
      if (n.includes('critical') || n.includes('berpikir kritis')) return { icon: Brain, color: '#a855f7' };
      if (n.includes('detail') || n.includes('berorientasi detail')) return { icon: Search, color: '#10b981' };
      if (n.includes('fast') || n.includes('pembelajar cepat')) return { icon: Zap, color: '#f59e0b' };
      if (n.includes('problem') || n.includes('penyelesaian masalah')) return { icon: Lightbulb, color: '#eab308' };
      if (n.includes('team') || n.includes('kerja sama tim')) return { icon: Users, color: '#3b82f6' };
      if (n.includes('time') || n.includes('manajemen waktu')) return { icon: Clock, color: '#f43f5e' };
      if (n.includes('ethic') || n.includes('etika kerja')) return { icon: ShieldCheck, color: '#64748b' };
    }

    if (skillType === 'hard') {
      if (n.includes('tax')) return { icon: ShieldCheck, color: '#f59e0b' };
      if (n.includes('finance') || n.includes('statement')) return { icon: TrendingUp, color: '#3b82f6' };
      if (n.includes('accounting') || n.includes('bookkeeping')) return { icon: Calculator, color: '#10b981' };
    ...
    if (n.includes('powerpoint')) return { icon: Presentation, color: '#D24726' };
  ...
  if (type === 'tech') {
    ...
    return (
      <div ref={boxRef} className="flex flex-col items-center group relative cursor-default perspective-1000">
        <motion.div 
        ...
        >
          {/* Front Face: Icon */}
          <div className={`absolute inset-0 backface-hidden rounded-2xl flex items-center justify-center p-3 border transition-all duration-300 ${
            isDark 
          ...
          <div className={`absolute inset-0 backface-hidden rounded-2xl flex items-center justify-center p-2 border transition-all duration-300 overflow-hidden rotate-y-180 ${
            isDark 
        ...
        <div className="h-6 mt-1 flex items-center justify-center overflow-visible">
          <span 
            className={`text-[9px] font-bold text-center whitespace-nowrap transition-all duration-300 transform translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 ${
              isRevealed || isDark ? 'translate-y-0 opacity-100 text-blue-400' : 'text-blue-600'
            }`}
          >
            {tool.name}
          </span>
  ...
  if (type === 'soft') {
    return (
      <motion.div whileHover={{ x: 3 }} className="flex items-center gap-3 p-1 transition-all duration-300">
        <div className="w-8 h-8 flex items-center justify-center shrink-0">
          <IconComponent size={18} style={{ color: style.color }} />
        </div>
        <span className={`text-xs font-bold tracking-tight transition-colors ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>{skill}</span>
      </motion.div>
    );
  }

  return (
    <motion.div whileHover={{ y: -3, scale: 1.02 }} className={`flex items-center gap-3 p-3 rounded-xl border transition-all duration-300 h-full ${isDark ? 'bg-slate-900/40 border-white/5 hover:border-blue-500/40 hover:bg-slate-800/60' : 'bg-white border-slate-200 shadow-sm hover:shadow-md hover:border-blue-400'}`}>
      <div className="w-9 h-9 md:w-10 md:h-10 rounded-lg flex items-center justify-center shrink-0" style={{ backgroundColor: `${style.color}20`, border: `1px solid ${style.color}40` }}>
        <IconComponent size={20} style={{ color: style.color }} />
      </div>
      <span className={`text-[10px] md:text-[11px] font-bold uppercase tracking-tight leading-tight ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>{skill}</span>
    </motion.div>
  );
}
  ...
  return (
    <motion.div 
      ...
      transition={{ 
        ...
        x: isHovering ? { type: "spring", stiffness: 450, damping: 30 } : { duration: 0.2 },
    ...
    >
      <div className="relative">
        {/* Very Slim UFO Body */}
        <div className={`w-14 h-3.5 rounded-full relative ${isDark ? 'bg-slate-200' : 'bg-slate-400'} shadow-xl border-b border-slate-500/50`}>
          {/* Aerodynamic Cockpit */}
          ...
          <div className="flex justify-around items-center h-full px-1.5">
            {[0, 1].map((i) => (
              <div key={i} className="w-0.5 h-0.5 rounded-full bg-blue-400 shadow-[0_0_5px_rgba(96,165,250,0.9)] animate-pulse" style={{ animationDelay: `${i * 0.4}s` }}></div>
            ))}
          </div>
        ...
        <motion.div 
          style={{ 
            ...
            background: 'linear-gradient(to bottom, rgba(56, 189, 248, 0.35) 0%, rgba(56, 189, 248, 0.08) 60%, transparent 100%)'
          }}
...
export function TechnicalCarousel({ tools, isDark, isScannerActive, globalMouseX }: any) {
  const containerRef = useRef<HTMLDivElement>(null);
  ...
  return (
    <div 
      ref={containerRef}
      className={`relative py-4 px-2 select-none min-h-[250px] w-full flex flex-col items-center justify-start transition-all duration-700 ${isScannerActive ? 'cursor-crosshair' : 'cursor-default'}`}
    >
      <div className="grid grid-cols-3 md:grid-cols-5 lg:grid-cols-3 place-items-center gap-y-10 gap-x-6 relative z-20 w-full max-w-4xl mx-auto">
...
export function CertCategoryCarousel({ categories, descriptions, isDark, viewCertBtnText, seeMoreBtnText }: any) {
  ...
  const prevZoom = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (zoomedItemIndex !== null) {
      setZoomedItemIndex((zoomedItemIndex - 1 + activeCategory.items.length) % activeCategory.items.length);
    }
  };
    ...
    <div className="w-full flex flex-col items-center">
      ...
      <div className="relative w-full h-[320px] md:h-[420px] flex items-center justify-center overflow-visible py-10">
        <div className="absolute top-1/2 left-0 right-0 flex justify-between px-2 md:px-4 z-30 -translate-y-1/2 pointer-events-none max-w-6xl mx-auto w-full">
          <button onClick={prev} className={`p-3 rounded-full border pointer-events-auto transition-all ${isDark ? 'bg-slate-800 border-white/10 text-white hover:bg-blue-600' : 'bg-white border-slate-200 text-slate-800 hover:bg-blue-600 hover:text-white'}`}><ChevronLeft size={24}/></button>
          <button onClick={next} className={`p-3 rounded-full border pointer-events-auto transition-all ${isDark ? 'bg-slate-800 border-white/10 text-white hover:bg-blue-600' : 'bg-white border-slate-200 text-slate-800 hover:bg-blue-600 hover:text-white'}`}><ChevronRight size={24}/></button>
        </div>
              ...
              return (
                <motion.div
                  ...
                  className={`absolute w-[240px] md:w-[320px] p-6 md:p-10 rounded-[2.5rem] border flex flex-col items-center text-center transition-all duration-300 overflow-hidden ${
                    isActive 
                      ? (isDark ? 'bg-slate-900 border-blue-500/50 shadow-[0_0_40px_rgba(255,255,255,0.1),0_0_30px_rgba(59,130,246,0.3)]' : 'bg-white border-blue-400 shadow-[0_0_40px_rgba(255,255,255,0.8),0_0_30px_rgba(59,130,246,0.2)]')
                ...
                >
                  ...
                  <div className="absolute inset-0 z-0">
                    <img 
                      ...
                      referrerPolicy="no-referrer"
                    />
                    <div className={`absolute inset-0 ${isDark ? 'bg-gradient-to-b from-transparent to-slate-900/40' : 'bg-gradient-to-b from-transparent to-white/40'}`} />
                  </div>

                  <div className={`relative z-10 w-16 h-16 md:w-20 md:h-20 rounded-2xl flex items-center justify-center border mb-6 transition-all ${
                    isActive 
                      ? (isDark ? 'bg-blue-500/20 border-blue-400 text-blue-400' : 'bg-blue-50 border-blue-200 text-blue-600 shadow-lg')
                      : (isDark ? 'bg-slate-800 border-white/5 text-slate-500' : 'bg-slate-50 border-slate-100 text-slate-400')
                  }`}>
                    <Icon size={isActive ? 36 : 28} />
                  </div>
                  
                  <h4 className={`relative z-10 text-base md:text-lg font-bold mb-5 tracking-tight leading-tight ${isDark && isActive ? 'text-white' : (isActive ? 'text-slate-900' : 'text-slate-500')}`}>
                    {cat.name}
                  </h4>
                  ...
                  {isActive && (
                    ...
                    >
                      {seeMoreBtnText}
                      <motion.div animate={{ rotate: expandedId === cat.id ? 180 : 0 }}>
                        <ChevronDown size={12} />
                      </motion.div>
                    </button>
                  )}
      ...
      <AnimatePresence>
        {expandedId && (
          ...
          >
            <div className={`p-8 md:p-14 rounded-[3.5rem] border backdrop-blur-xl mt-4 mb-20 ${isDark ? 'bg-slate-900/80 border-white/10 shadow-2xl' : 'bg-white/95 border-slate-200 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)]'}`}>
               <motion.p 
                 initial={{ opacity: 0 }} 
                 animate={{ opacity: 1 }} 
                 transition={{ delay: 0.15, duration: 0.3 }}
                 className={`text-sm md:text-base leading-relaxed text-center mb-16 mx-auto max-w-3xl italic font-medium opacity-80 ${isDark ? 'text-slate-200' : 'text-slate-700'}`}
               >
            ...
            >
              ...
              <div className="w-full h-full overflow-hidden rounded-2xl border border-white/10 shadow-[0_0_100px_rgba(59,130,246,0.15)] bg-slate-900 group/modal relative">
                ...
                <img 
                  ...
                  alt={activeCategory.items[zoomedItemIndex].title || "Certificate"} 
  ...
  return (
    <div className="relative w-full py-10 flex flex-col items-center overflow-hidden bg-blue-500/5 rounded-3xl mt-4">
      <div className="flex items-center justify-center gap-4 md:gap-8 w-full max-w-7xl px-4">
        <button onClick={prev} className={`p-2 rounded-full border z-20 ${isDark ? 'bg-slate-800 border-white/10 text-white' : 'bg-white border-slate-200 text-slate-800'} hover:bg-blue-600 hover:text-white transition-all`}><ChevronLeft size={20} /></button>
        <div className="flex items-center justify-center relative w-full h-[220px] md:h-[450px]">
          <AnimatePresence initial={false}>
            {items.map((cert: any, i: number) => {
              const isCenter = i === index;
              const isVisible = Math.abs(i - index) <= 1 || (index === 0 && i === items.length - 1) || (index === items.length - 1 && i === 0);
              if (!isVisible) return null;
              const offset = i - index;
              const xValue = offset * (windowWidth < 768 ? 160 : 380);
              return (
                <motion.div key={cert.title} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: isCenter ? 1 : 0.4, scale: isCenter ? 1.15 : 0.85, x: xValue, zIndex: isCenter ? 10 : 5 }} className="absolute w-[200px] md:w-[450px] aspect-[4/3] rounded-2xl overflow-hidden shadow-2xl border border-white/10 bg-slate-900">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-transparent to-transparent flex flex-col justify-end items-start p-4 md:p-6 text-left">
                    <h5 className="text-white text-[9px] md:text-sm font-bold leading-tight mb-0 capitalize tracking-normal">{cert.title}</h5>
                  </div>
                </motion.div>
              );
            })}
        ...
        <button onClick={next} className={`p-2 rounded-full border z-20 ${isDark ? 'bg-slate-800 border-white/10 text-white' : 'bg-white border-slate-200 text-slate-800'} hover:bg-blue-600 hover:text-white transition-all`}><ChevronRight size={20} /></button>
      </div>
    </div>
  );
}

...
export function Projects({ isDark, t, lang }: ProjectsProps) {
  const [projectsFromDB, setProjectsFromDB] = useState<any[]>([]);
  ...
  category: lang === 'id' && p.category_id ? p.category_id : (p.category || 'Project'),
  desc: lang === 'id' && p.desc_id ? p.desc_id : (p.description || ''),
  details: lang === 'id' && p.tags_id && p.tags_id.length > 0
    ? p.tags_id.filter((t: string) => t.trim() !== '')
    : Array.isArray(p.tags) ? p.tags.filter((t: string) => t.trim() !== '') : [],
  image: p.image_url || '',
  link: p.live_url || p.github_url || '#'
}))

  // Kalau ada data dari Supabase, pakai itu. Kalau kosong, pakai data statis dari constants
  const projectsToShow = formattedProjects.length > 0 ? formattedProjects : t.projectsData
